...
- `TableLayout` and `FindLayout` describe the active information and Node-find
  surfaces.
...
`LayoutBackground.color`, `image`, `svg`, `guides`, or `conditional`; these factories
...
application callback boundary. A `RandomLayoutBackground` configured anywhere
beneath a Graph background's conditional wrappers resolves independently and
stably for every rendered Node, then clips to the same projected Node path.
...
Its optional `background` list paints above the Layout background and Node fill
but below the Node border and caption. A non-null list replaces the inherited
Node background list; use an empty list to suppress inherited artwork. Match
per-Node icon artwork with `LayoutCondition.hasLabelsInCurrentNode(...)`.
...
metrics, font features, shadow effects, and an optional `LayoutTextFlow`.
Horizontal flow is the shared default. Vertical flow independently configures
top-to-bottom or bottom-to-top progression and rotated or upright glyphs;
Flutter's `TextDirection` remains reserved for bidi shaping. Flame uses this
flow for both intrinsic text height and flat/projected panel painting, advancing
panoramic vertical captions along the resolved surface's vertical basis.
LG Ergo applies vertical flow to every `ColumnLayout` and horizontal flow to
every `RowLayout`; the child Row override wins when these compositions nest.
Use `LayoutText.none()` to suppress
...
components. `FindLayoutComponent` is a high-priority Flame component owned by
the game. It owns Find visibility and keyboard actions. `FindInputOverlay`
hosts the native Flutter editable control and maps its paint and hit testing to
the exact input quadrilateral sampled from the Flame-resolved surface. The
submitted query is stored in `searchStateProvider`. The corresponding
action-plane button remains a Flame-rendered layout element. Its frame and
sizing come from LG Ergo's `safe-area/find-layout` `FindLayout`, which fills the
resolved safe area and paints above ordinary scene/action-panel content.
...
Riverpod. `FindLayoutComponent` subscribes through Flame's keyboard-handler
system and invokes callbacks supplied by the screen. Command-F opens Find,
Command-R refreshes every currently watched Fan `NodeTree` provider,
Command-C copies the last selected Node slug, and Escape clears selected Nodes
and hides Find.

Find submission is shared by its action-plane button and keyboard Enter. The
normalized value is stored in `searchStateProvider`, and
...
Configure `fallbackNode` as its frontend fallback. A successful empty lookup
...
Find is visible, its native input consumes Enter to submit the Node query or
select the highlighted result instead of creating a Node.
