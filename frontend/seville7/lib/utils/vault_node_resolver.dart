...
class VaultNodeResolver {
  ...
  ResolvedVaultNode? findLinkedNode(
    String? value, {
    required List<String> layoutPath,
  }) {
    final nodes = linkedNodes(value);
    return nodes.isEmpty ? null : resolve(nodes.first, layoutPath: layoutPath);
  }

  List<ResolvedVaultNode> findLinkedNodes(
    String? value, {
    required List<String> layoutPath,
  }) => [
    for (final node in linkedNodes(value))
      resolve(node, layoutPath: layoutPath),
  ];
